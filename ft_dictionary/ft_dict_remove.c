...
{
	ft_pair_delete((*node)->data, dict->keydel, dict->valdel);
	free((*node));
	*node = NULL;
}
...
static t_bstree	*helper(t_bstree **tree, t_pair *pair, t_dict *dict)
{
	...
	else
	{
		...
		ft_pair_delete((*tree)->data, dict->keydel, dict->valdel);
		(*tree)->data = ft_pair_copy(node->data, dict->keycpy, dict->valcpy);
